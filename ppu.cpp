...
void Ppu::draw_line() {
  ...
  u16 tile_begin_address = bg_address + ((aligned_line / 8) * (16 * 32)) + ((x_index / 8) * 16);

  std::array<u8, 8> line_buffer{0};

  for (int i = 0; i <= 20 && (i + x_index) < 32; ++i) {
	render_line_of_tile(tile_begin_address + (i * 16), line % 8, line_buffer);
  ...
  for (auto &spr : state.spritesOnLine) {
	int top_line = spr.y_pos - vert_size;
	int line = std::abs((*mem_mgr)[LY] - top_line);

	render_line_of_tile(spr.address, line, line_buffer);
