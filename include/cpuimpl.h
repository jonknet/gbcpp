...
struct CpuState {
  ...
  s16 lastop;
  bool lastop_cb;
  CpuState() : cycles(0), ime(true), running(false), halted(false), stopped(false), lastop(0),lastop_cb(false) {};
};
...
class Cpu::CpuImpl {
 ...
 protected:
  ...
  void jr(bool cond) {
	log_buffer = fmt::format("{:64s}",fmt::format("{:s} {:d} : Oldr.pc {:x} : Newr.pc {:x}",__func__, cond, r.pc, r.pc + ((cond) ? (s8)get8(r.pc + 1) : 0) + 2))  + log_buffer;
	r.pc += ((cond) ? (static_cast<s8>(get8(r.pc + 1))) : 0);
  }

  void jp(bool cond) {
	log_buffer = fmt::format("{:64s}",fmt::format("{:s} {:d} : Oldr.pc {:x} : Newr.pc {:x}",__func__, cond, r.pc, ((cond) ? get16(r.pc + 1) : r.pc)))  + log_buffer;
	r.pc = ((cond) ? (get16(r.pc + 1) + 3) : r.pc);
  }

  void call(bool cond) {
	log_buffer = fmt::format("{:64s}",fmt::format("{:s} {:d} : Oldr.pc {:x} : Newr.pc {:x}",__func__, cond, r.pc, get16(r.pc + 1)))  + log_buffer;
	if (cond) {
	  push16(r.pc + 3);
	  r.pc = get16(r.pc + 1) - 3;
	}
  ...
  void ret(bool cond) {
	log_buffer = fmt::format("{:64s}",fmt::format("{:s} {:d} : Oldr.pc {:x} : Newr.pc {:x}",__func__, cond, r.pc, peek16()))  + log_buffer;
	r.pc = ((cond) ? pop16() - 1 : 0);
  }
  ...
  void bit(u16 v, int bit_num) {
	log_buffer = fmt::format("{:64s}",fmt::format("{:s} ({:x}),{:d} = {}", __func__,v, bit_num, (isbitset(v, bit_num)) ? "true" : "false"))  + log_buffer;
 ...
 public:
  void lookup_and_execute() {
    log_buffer.clear();
    log_buffer = fmt::format("\t\t[PC:{:04x} A:{:02x} B:{:02x} C:{:02x} D:{:02x} E:{:02x} H:{:02x} L:{:02x} SP:{:04x} [{:04x} {:04x}]]", r.pc,
							 *r.a,
							 *r.b,
							 *r.c,
							 *r.d,
	...
	switch (s.lastop) {
	  ...
	  case 0xCB:
		...
		switch ((*m)[r.pc + 1]) {
		  ...
		  case 0xFF:set(r.a, 7);
			break;
		  default:spdlog::warn("Unknown CB or.pcode {0:x}", (*m)[r.pc + 1]);
			assert(0);
		}
		break;
